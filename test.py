...
import operator
import json
from model import entity_dic, relation_dic
...
def dataloader(entity_file, relation_file, test_file):
    ...
    with codecs.open(test_file) as t_f:
        ...
        for line in lines:
            ...
            if len(triple) != 3:
                continue
            h_ = entity_dic[triple[0]]
            t_ = entity_dic[triple[1]]
            r_ = relation_dic[triple[2]]
...
class Test:
    ...
    def rank(self):
        ...
        for triple in self.test_triple:
            ...
            total_len = len(self.test_triple)
            remain = total_len - counter
...
if __name__ == '__main__':
    _, _, train_triple = load_data("FB15k")

    entity_dict, relation_dict, test_triple = \
        dataloader("entity_temp_trained", "relation_temp_trained",
                   "FB15k\\test.txt")
